...
async function createUser(username: string, token: string) {
  ...
  const tihldeMemberships = await fetch(
    `https://api.tihlde.org/users/${username}/memberships/`,
    {
      headers: {
        "X-Csrf-Token": token,
      },
  ...
  // Get user
  const tihldeuser: TihldeUser = await fetch(
    `https://api.tihlde.org/users/${username}/`,
    {
      headers: {
        "X-Csrf-Token": token,
      },
  ...
  providers: [
    Credentials({
      ...
      authorize: async (credentials) => {
        ...
        role = user?.role
          ? roles[updatedUser.role as "ADMIN" | "MEMBER" | "GUEST" | "LEADER"]
          : roles.GUEST;
    ...
    async jwt({ token, user }) {
      ...
      if (name) {
        ...
        if (u) {
          token.role = u?.role
            ? roles[
                u.role as "ADMIN" | "MEMBER" | "GUEST" | "LEADER" | "INDEKS"
              ]
            : roles.GUEST;
        }
      }
