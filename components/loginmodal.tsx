...
export default function LoginModal({ onFinish, onCancel }: { onFinish?: () => void, onCancel?: () => void }) {
    ...
    const handleLogin = async () => {
        ...
        signIn("credentials", { username, password, redirect: false }).then((res) => {
            if (res?.error) {
                setWrongCredentials("Feil brukernavn eller passord brir!");
    ...
    return (
        <Modal onClose={onCancel}>
            ...
            <div className="flex flex-col space-y-3 mt-6 mb-4">
                ...
                <TextInput placeholder="Password" className="h-12" onChange={setPassword} onEnterBlur onEnterClear onSubmit={(s) => { setPassword(s); handleLogin(); }} type="password" ref={passwordRef} />
            </div>
            {wrongCredentials ?
                <div className="text-red-600 text-center pb-2">
            ...
            <div className="flex w-full justify-center items-center">
                <Button className=" min-w-28" variant="primary" onClick={handleLogin}>{loginButtonLoading ?
